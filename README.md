...
`src/classifier.ts` に読みやすい正規表現ルールを集約しています。RESET_ANNOUNCED / RESET_HINT / BANKED_RESET / LIMIT_CHANGE が通知対象。CANDIDATEとIRRELEVANTは保存のみです。通常のCodex/Astra/Solの技術議論、単独の目の絵文字、Gitやキャッシュのresetは対象外。引用ブロックは本人の本文から除き、識別できるリポストは通知しません。返信判別はRSSで得られる情報に限られます。

初回baselineの全89投稿を固定した回帰テストで、到着・実施済みreset、Banked Reset配布、5時間枠、reset buttonの匂わせを検証しています。[評価結果と判定の境界](docs/classifier-regression.md)を参照してください。分類器の更新で、保存済みの過去投稿を再分類・再通知することはありません。
