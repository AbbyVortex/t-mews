...
import { notify, titles, alertMessage } from './pushover';
...
export async function monitor(env: Env, now=Math.floor(Date.now()/1000), fetcher: Fetcher=fetch) {
 ...
 for(const post of unique) {
  ...
  const historical=bootstrap || (post.publishedAt!==null && (post.publishedAt<=state.baselineAt || post.publishedAt<now-14*86400));
 ...
 for(const row of pending.results) {
  const post:Item={id:row.id,xStatusId:row.x_status_id,url:row.canonical_url,fingerprint:row.fingerprint,text:row.text,publishedAt:row.published_at,source:row.source,kind:row.kind};
  const classification=row.classification as ReturnType<typeof classify>['classification'];
  await notify(env,`post:${post.id}`,'alert',titles[classification]!,alertMessage(post,classification,row.detection_latency_seconds),now,fetcher,post,classification);
 }
