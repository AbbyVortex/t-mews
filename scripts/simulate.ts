...
const xml=process.argv[2]?readFileSync(process.argv[2],'utf8'):await (await fetch('https://fxtwitter.com/thsottiaux/feed.xml',{signal:AbortSignal.timeout(8000)})).text();
const expected=(await parseFeed(xml,{name:'fxtwitter',url:'https://fxtwitter.com/thsottiaux/feed.xml'})).length;
...
const mf=new Miniflare({modules:true,script:patched,compatibilityDate:'2026-07-30',d1Databases:['DB'],bindings:{MONITOR_ENABLED:'true',SOURCES_JSON:JSON.stringify([{name:'fxtwitter',url:'https://fxtwitter.com/thsottiaux/feed.xml'}]),PUSHOVER_USER_KEY:'fixture',PUSHOVER_APP_TOKEN:'fixture'}});
try {
